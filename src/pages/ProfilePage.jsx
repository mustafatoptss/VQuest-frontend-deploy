  ...
  return (
    <div>
      ...
      <div className="stats-grid mb-3">
        <div className="stat-card"><div className="stat-icon">🏆</div><div className="stat-value">{p.totalScore || 0}</div><div className="stat-label">Toplam Puan</div></div>
        <div className="stat-card"><div className="stat-icon">🎮</div><div className="stat-value">{p.gamesPlayed || 0}</div><div className="stat-label">Oyun Sayısı</div></div>
        <div className="stat-card"><div className="stat-icon">✅</div><div className="stat-value">{p.correctRate || 0}%</div><div className="stat-label">Doğruluk Oranı</div></div>
      </div>
      ...
      {tab === 'danger' && (
        <div className="card" style={{ border: '1px solid rgba(255,82,82,0.3)' }}>
          <h3 style={{ color: 'var(--danger)', marginBottom: '0.5rem' }}>⚠️ Hesap Silme</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1rem' }}>Bu işlem geri alınamaz. Tüm verileriniz silinecektir.</p>
          <button className="btn btn-danger" onClick={handleDeleteAccount}>🗑️ Hesabımı Sil</button>
        </div>
      )}
    </div>
  );
